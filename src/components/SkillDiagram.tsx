...
export default function SkillDiagram({ spec, className }: { spec: SkillDiagramSpec; className?: string }) {
  ...
  if (spec.kind === 'strike') {
    ...
    const fid = `${uid}-flight`
    return (
      <svg viewBox="0 0 220 130" className={className}>
        <defs>
          <marker id={`${uid}-a`} markerWidth="6" markerHeight="6" refX="4" refY="3" orient="auto">
            <path d="M0,0 L6,3 L0,6 Z" fill="#1fd17a" />
          </marker>
          <path id={fid} d={flightPath(spec.flight)} />
        </defs>
        ...
        <use href={`#${fid}`} fill="none" stroke="#1fd17a" strokeWidth={2.5} markerEnd={`url(#${uid}-a)`} />
        {/* Ball travelling along the flight path */}
        <circle r={4} fill="#f4f7f5" stroke="#0c130f" strokeWidth={1.5}>
  ...
  // move diagram: ball + numbered arrows, with a ball travelling each path
  const bx = 110
  const by = 92
  return (
    <svg viewBox="0 0 220 150" className={className}>
      <defs>
        <marker id={`${uid}-m`} markerWidth="6" markerHeight="6" refX="4" refY="3" orient="auto">
        ...
        {spec.arrows.map((a, i) => {
          ...
          return <path key={i} id={`${uid}-mv${i}`} d={`M${x1},${y1} Q${mx},${my} ${x2},${y2}`} />
        })}
      </defs>
      ...
      {spec.arrows.map((a, i) => {
        ...
        return (
          <g key={i}>
            <use href={`#${uid}-mv${i}`} fill="none" stroke="#f4c95d" strokeWidth={2.5} strokeDasharray={a.dashed ? '4 3' : undefined} markerEnd={`url(#${uid}-m)`} />
            <circle r={5} fill="#fff" stroke="#0c130f" strokeWidth={1.2}>
              <animateMotion dur="1.8s" begin={`${i * 0.5}s`} repeatCount="indefinite" keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                <mpath href={`#${uid}-mv${i}`} />
              </animateMotion>
            </circle>
